    ...
    try {
      ...
      if (!error && user) {
        ...
        if (userRole === Role.TALENT) {
          ...
          const redirectUrl = isOnboarded ? '/talent/dashboard' : '/talent/onboarding'
          return NextResponse.redirect(new URL(redirectUrl, request.url))
        } else if (userRole === Role.EMPLOYER) {
          // Check if onboarded to determine exact redirect
          const isOnboarded = user.user_metadata?.is_onboarded
          const redirectUrl = isOnboarded ? '/employer/dashboard/home' : '/employer/onboarding'
          return NextResponse.redirect(new URL(redirectUrl, request.url))
        }
      }
      ...
      if (!error && user) {
        ...
        if (isOnboarded) {
          if (userRole === Role.TALENT) {
            ...
            return NextResponse.redirect(new URL('/employer/dashboard/home', request.url))
          }
        }
      }
  ...
  try {
    ...
    if (pathname.startsWith('/talent')) {
      if (userRole !== Role.TALENT) {
        ...
        return NextResponse.redirect(new URL('/auth/talent/login', request.url))
      }
      ...
      if (pathname.startsWith('/talent/dashboard')) {
        ...
        const isOnboarded = user.user_metadata?.is_onboarded
        if (!isOnboarded) {
          return NextResponse.redirect(new URL('/talent/onboarding', request.url))
        }
      }
    }
