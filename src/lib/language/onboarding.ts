export const onboardingTranslations = {
  en: {
    // Progress indicators
    stepOf: "Step",
    of: "of",
    complete: "Complete",
    
    // Common buttons
    back: "Back",
    next: "Next →",
    skipForNow: "Skip for now",
    completeSetup: "Complete Setup",
    completeSetupArrow: "Complete Setup →",
    
    // Step titles and descriptions
    locationTitle: "Where would you like to work?",
    locationDescription: "Enter your preferred location",
    locationPlaceholder: "e.g., San Francisco, CA or Remote",
    
    industriesTitle: "What are your top 3 target industries?",
    industriesDescription: "Selected:",
    
    opportunitiesTitle: "What opportunities are you looking for?",
    opportunitiesDescription: "Select all that apply",
    
    aboutTitle: "Tell us about yourself",
    aboutDescription: "Write a brief bio to help us match you better",
    aboutPlaceholder: "I'm a computer science student passionate about AI and machine learning. I enjoy building projects that solve real-world problems...",
    
    // About yourself specific
    characters: "characters",
    minimum: "minimum",
    resumeUploaded: "Resume uploaded ✓",
    aiGenerate: "AI Generate",
    generating: "Generating...",
    
    // Toast messages
    bioGeneratedSuccess: "Bio generated successfully!",
    bioGeneratedError: "Failed to generate bio",
    resumeNotAvailable: "Resume text not available. Please try refreshing the page.",
    bioGenerationError: "An error occurred while generating the bio",
    
    // Completion messages
    profileCreatedSuccess: "Profile created successfully!",
    userNotAuthenticated: "User not authenticated",
    resumeUploadError: "Failed to upload resume after multiple attempts",
    profileSaveError: "Failed to save profile after multiple attempts",
    profileVerificationFailed: "Profile creation verification failed. Please try again.",
    operationTimedOut: "The process is taking longer than expected. Please try again.",
    unexpectedError: "An unexpected error occurred. Please try again.",
    finalizingProfile: "Finalizing your profile... Please wait.",
    
    // Opportunity labels (for display, not database values)
    fullTimeLabel: "Full-time Jobs",
    internshipsLabel: "Internships",
    bootcampsLabel: "Bootcamps",
    hackathonsLabel: "Hackathons",
  },
  ar: {
    // Progress indicators
    stepOf: "الخطوة",
    of: "من",
    complete: "مكتمل",
    
    // Common buttons
    back: "رجوع",
    next: "التالي ←",
    skipForNow: "تخطي الآن",
    completeSetup: "إكمال الإعداد",
    completeSetupArrow: "إكمال الإعداد ←",
    
    // Step titles and descriptions
    locationTitle: "أين تريد العمل؟",
    locationDescription: "أدخل الموقع المفضل لديك",
    locationPlaceholder: "مثال: الرياض، السعودية أو عن بُعد",
    
    industriesTitle: "ما هي أهم 3 صناعات تستهدفها؟",
    industriesDescription: "المحدد:",
    
    opportunitiesTitle: "ما هي الفرص التي تبحث عنها؟",
    opportunitiesDescription: "اختر جميع ما ينطبق",
    
    aboutTitle: "أخبرنا عن نفسك",
    aboutDescription: "اكتب نبذة مختصرة لمساعدتنا في مطابقتك بشكل أفضل",
    aboutPlaceholder: "أنا طالب علوم حاسب شغوف بالذكاء الاصطناعي والتعلم الآلي. أستمتع ببناء مشاريع تحل مشاكل حقيقية...",
    
    // About yourself specific
    characters: "أحرف",
    minimum: "الحد الأدنى",
    resumeUploaded: "تم رفع السيرة الذاتية ✓",
    aiGenerate: "توليد بالذكاء الاصطناعي",
    generating: "جارٍ التوليد...",
    
    // Toast messages
    bioGeneratedSuccess: "تم إنشاء النبذة بنجاح!",
    bioGeneratedError: "فشل في إنشاء النبذة",
    resumeNotAvailable: "نص السيرة الذاتية غير متاح. يرجى محاولة تحديث الصفحة.",
    bioGenerationError: "حدث خطأ أثناء إنشاء النبذة",
    
    // Completion messages
    profileCreatedSuccess: "تم إنشاء الملف الشخصي بنجاح!",
    userNotAuthenticated: "المستخدم غير مصادق عليه",
    resumeUploadError: "فشل رفع السيرة الذاتية بعد محاولات متعددة",
    profileSaveError: "فشل حفظ الملف الشخصي بعد محاولات متعددة",
    profileVerificationFailed: "فشل التحقق من إنشاء الملف الشخصي. يرجى المحاولة مرة أخرى.",
    operationTimedOut: "العملية تستغرق وقتًا أطول من المتوقع. يرجى المحاولة مرة أخرى.",
    unexpectedError: "حدث خطأ غير متوقع. يرجى المحاولة مرة أخرى.",
    finalizingProfile: "جارٍ إنهاء إعداد ملفك الشخصي... يرجى الانتظار.",
    
    // Opportunity labels (for display, not database values)
    fullTimeLabel: "وظائف بدوام كامل",
    internshipsLabel: "تدريبات",
    bootcampsLabel: "معسكرات تدريبية",
    hackathonsLabel: "هاكاثونات",
  }
};

